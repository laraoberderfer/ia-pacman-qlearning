...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getReward(self, state, action, nextState):
        ...
        Obtenha recompensa por transição de estado, ação, nextState.

         Observe que a recompensa depende apenas do estado ser
         partiu (como nos exemplos do livro R + N, que mais ou
         menos usar esta convenção).
        """
        if state == self.grid.terminalState:
            return 0.0
        x, y = state
